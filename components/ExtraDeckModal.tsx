        ...
        try {
            ...
            matIds.forEach(id => {
                ...
                if (c.subType?.includes('PENDULUM')) {
                    moveCard(id, 'EXTRA_DECK', 0, undefined, true); // Suppress log
                } else {
                    moveCard(id, 'GRAVEYARD', 0, undefined, true); // Suppress log
                }
            });
            ...
            const materialsLabel = matIds.map(id => getCardName(cards[id], useGameStore.getState().language)).join('＋');
            moveCard(linkCardId, zoneType, zoneIndex, undefined, false, true, `link:${materialsLabel}`);
